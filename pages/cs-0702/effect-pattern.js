...
export default function EffectPattern() {
  ...
  useEffect(() => {
    console.log('首次渲染(render)之後(after)，執行一次第一傳入參數函式中的程式碼，之後不會再執行')
  }, [])
  ...
  useEffect(() => {
    console.log('首次渲染之後，執行一次第一傳入參數函式中的程式碼。之後當相依變數有更動時，會再執行一次其中程式碼。')
  }, [total])
  // ^^^^^^^ total加入相依變數陣列中，代表監聽total狀態的更動(change)事件
